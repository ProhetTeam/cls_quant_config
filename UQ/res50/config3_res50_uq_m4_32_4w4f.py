...
quant_transformer = dict(
    ...
    quan_policy=dict(
        Conv2d=dict(type='APOTQuantConv2d', bit_w=4, bit_a=4, power=False),
        Linear=dict(type='APOTQuantLinear', bit_w=4, bit_a=4, power=False)
        ),
    special_layers = dict(
        layers_name = [
            'backbone.conv1',
...
# optimizer
num_nodes = 3
optimizer = dict(type='SGD', lr=1e-2 * num_nodes, momentum=0.9, weight_decay=0.25e-4)
...
work_dir = '/data/work_dirs/UQ/res50/config3_res50_lsq_m2_64_4w4f'
workflow = [('train', 1)]

load_from = 'thirdparty/modelzoo/res50.pth'
resume_from = '/data/work_dirs/UQ/res50/config3_res50_lsq_m2_64_4w4f/latest.pth'
cpu_only=True
